 ...
 *
 * Robinhood Chain's public RPC (rpc.mainnet.chain.robinhood.com) is on the Indonesian
 * TrustPositif block list, so a browser or wallet on such a network cannot read a balance or
 * broadcast a payment. `chains.ts` hands the browser this route instead; the server, which
 * sits outside that filter, forwards to the chain's `serverRpc`. It is also the one place to
 * swap the public endpoint for a dedicated provider later without a client change.
 *
 * It is a relay, not an open proxy: only the methods a wallet dapp needs are forwarded,
 * bodies and batches are capped, and each IP gets a small token bucket. Responses pass
 * through untouched so viem sees exactly what the chain said.
 */
...
// Token bucket per IP: RPC_RELAY_RATE requests per second, bursting to RPC_RELAY_BURST.
// In-memory, so per server instance; enough for a single-box deploy.
...
export async function POST(request: NextRequest, { params }: { params: { chainId: string } }) {
  const chain = getChain(Number(params.chainId));
  if (!chain) return respond(rpcError(null, -32000, `chain ${params.chainId} is not enabled`), 404);
  ...
  // A refused call in a batch is answered in its slot, not forwarded.
...
async function forward(
  ...
  try {
    ...
    try { json = JSON.parse(text); } catch {
      // A filter page or a load-balancer 502 is not JSON-RPC; surface it as a gateway error.
